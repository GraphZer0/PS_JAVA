...
public class DeliveryCostCalculator {
    ...
    public double calcatale(double distance, String size, boolean fragile, String workload) {

        double cost = 0;

        if (fragile && distance > 30) {
            throw  new IllegalArgumentException(
                    "Хрупкие грузы нельзя перевозить на растояние более 30 км");
        }
        ...
        if ("large".equalsIgnoreCase(size)) {
            ...
            cost += 100;}
        else { throw  new IllegalArgumentException("Некорректный размер груза"); }
